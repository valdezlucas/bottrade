...
def get_data(ticker):
    ...
    df = yf.download(
        ticker, start="2010-01-01", end="2026-02-22", interval="1d", progress=False
    )
    if df.empty:
        return None
...
def calculate_metrics(trades, equity, years):
    ...
    sortino = (
        (np.mean(daily_returns) / np.std(neg_ret) * np.sqrt(252))
    ...
    return {
        ...
        "WR": win_rate * 100,
        "PF": (
            np.sum(pnls[pnls > 0]) / abs(np.sum(pnls[pnls <= 0]))
            if any(pnls <= 0)
            else 99
        ),
        "n_trades": len(trades),
...
def run_backtest(df_feat, art, feature_cols, start_date, end_date, cfg):
    ...
    for i in range(len(df_p) - LOOKAHEAD):
        ...
        atr = (
            df_p["ATR"].iloc[i]
            if "ATR" in df_p.columns and not np.isnan(df_p["ATR"].iloc[i])
            else entry * 0.005
        )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ model_multi.joblib cargado | classes=[0,1,2] | threshold={art.get('threshold', '?')} | features={len(feature_cols)}\n"
        )
    ...
    for name, cfg in PAIRS.items():
        ...
        v_trades, v_equity = run_backtest(
            df_feat, art, feature_cols, SPLITS["VAL"][0], SPLITS["VAL"][1], cfg
        )
        ...
        o_trades, o_equity = run_backtest(
            df_feat, art, feature_cols, SPLITS["OOS"][0], SPLITS["OOS"][1], cfg
        )
        ...
        results[name] = {
            ...
            "MC": mc,
            "ROBUST": is_robust,
            "REASON": ("OVERFIT " if overfit else "") + ("FRAGILE" if fragile else "")
            or "STABLE",
        }

        print(f"\n  {'─'*50}")
        print(
            f"  VAL 2019-21 | PF: {v_m['PF']:>6.2f} | WR: {v_m['WR']:>5.1f}% | Sharpe: {v_m['Sharpe']:>5.2f} | Exp: ${v_m['Exp']:>7.2f} | Trades: {v_m['n_trades']}"
        )
        print(
            f"  OOS 2022-26 | PF: {o_m['PF']:>6.2f} | WR: {o_m['WR']:>5.1f}% | Sharpe: {o_m['Sharpe']:>5.2f} | Exp: ${o_m['Exp']:>7.2f} | Trades: {o_m['n_trades']}"
        )
        print(
            f"  Monte Carlo | Mean: ${mc['mean']:.0f} | P5: ${mc['p5']:.0f} | P95: ${mc['p95']:.0f} | P(Ruina): {mc['prob_ruin']:.1f}%"
        )
    ...
    print(f"{'='*80}")
    print(
        f"{'Par':<10} | {'VAL PF':>7} | {'OOS PF':>7} | {'VAL WR':>7} | {'OOS WR':>7} | {'MaxDD':>7} | {'Sharpe':>6} | Status"
    )
    print(f"{'─'*80}")
    ...
    for name, res in results.items():
        ...
        print(
            f"{name:<10} | {v['PF']:>7.2f} | {o['PF']:>7.2f} | {v['WR']:>6.1f}% | {o['WR']:>6.1f}% | {o['MaxDD']:>6.1f}% | {o['Sharpe']:>6.2f} | {st}"
        )
    ...
    print(
        f"  → Si OOS es positivo: Canary Live 30 días en Demo (mismo broker, mismo sizing)."
    )
