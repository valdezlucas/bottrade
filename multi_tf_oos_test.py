...
def download_1h_data(ticker, days=720):
    ...
    try:
        ...
        df = df.reset_index()
        if "Datetime" in df.columns: df = df.rename(columns={"Datetime": "Datetime"})
...
def run_backtest_tf(df_feat, art, feature_cols, cfg):
    ...
    for i in range(len(df_p) - LOOKAHEAD):
        ...
        atr = df_p["ATR"].iloc[i] if not np.isnan(df_p["ATR"].iloc[i]) else entry * 0.005
...
def main():
    ...
    for tf in ["4h", "1h"]:
        ...
        for name, cfg in PAIRS.items():
            ...
            if m:
                results[name] = m
                print(f"  {name:6s} | PF: {m['PF']:>5.2f} | WR: {m['WR']:>5.1f}% | Sharpe: {m['Sharpe']:>5.2f} | Trades: {m['n_trades']}")
            else: print(f"  {name:6s} | ⚠️ Sin trades")
