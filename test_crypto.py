...
def main():
    ...
    parser.add_argument(
        "--tf", type=str, choices=["1d", "4h", "1h", "ALL"], default="ALL"
    )
    ...
    for tf in tfs:
        ...
        for ticker in CRYPTO_PAIRS:
            ...
            if m:
                results[ticker] = m
                print(
                    f"  {ticker:10s} | PF: {m['PF']:>5.2f} | WR: {m['WR']:>5.1f}% | Sharpe: {m['Sharpe']:>5.2f} | Trades: {m['n_trades']}"
                )
            else:
                print(f"  {ticker:10s} | ⚠️ Sin trades")
