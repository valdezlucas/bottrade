        ...
        try:
            ...
            for i in range(len(df) - 20):  # Dejar margen para simular
                ...
                trade = {
                    ...
                    "sl": float(row["Close"] - row.get("potential_loss", row["Close"]*0.01)) if signal == "BUY" else float(row["Close"] + row.get("potential_loss", row["Close"]*0.01)),
                    "tp": float(row["Close"] + row.get("potential_win", row["Close"]*0.015)) if signal == "BUY" else float(row["Close"] - row.get("potential_win", row["Close"]*0.015)),
            ...
            emoji = "✅" if wr > 50 else "❌"
            print(f"  {emoji} {pair}: {n_signals} trades | {wins}W {losses}L {expired}E | WR: {wr:.1f}%")
