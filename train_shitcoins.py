...
def engineer_shitcoin_features(df):
    ...
    df["ret_24h"] = df["Close"].pct_change(
        periods=24
    )  # Suponiendo 1H tf, para 4H seria pct_change(6) adaptado dinamicamente no importa, dejamos pct simple
...
def label_reversals(df, lookahead=12):
    ...
    for i in range(len(df) - lookahead):
        ...
        atr = (
            df["ATR"].iloc[i]
            if "ATR" in df.columns and not np.isnan(df["ATR"].iloc[i])
            else entry * 0.03
        )
        ...
        pump_magn = (
            df["pump_4_bars"].iloc[i] if not np.isnan(df["pump_4_bars"].iloc[i]) else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--tf", type=str, choices=["1d", "4h", "1h", "ALL"], default="1h"
    )
    ...
    for tf in tfs_to_run:
        ...
        print(
            f"    📏 Clases balanceadas -> HOLD: {len(hold_sample)}, BUY REV: {len(buy_df)}, SELL REV: {len(sell_df)}"
        )
        ...
        calibrated_model = CalibratedClassifierCV(
            estimator=lgbm, method="isotonic", cv=3
        )
