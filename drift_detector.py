...
def rolling_kl(df_features, ref_period_end_idx, window=250, bins=50, threshold=0.2):
    ...
    kl_df = pd.DataFrame(
        index=df_features.index[ref_period_end_idx + window - 1 :],
    ...
    for i in range(ref_period_end_idx + window - 1, len(df_features)):
        ...
        for col in df_features.columns:
            ...
            kl_df.iloc[
                i - (ref_period_end_idx + window - 1), kl_df.columns.get_loc(col)
            ] = kl
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_df = pd.concat(
            [df_feat.iloc[:split_idx], df_feat.iloc[split_idx : split_idx + 255]]
        )
        kl_df, alerts = rolling_kl(
            test_df, ref_period_end_idx=split_idx, window=250, threshold=0.2
        )
