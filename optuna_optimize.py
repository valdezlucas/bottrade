...
class TradingObjective:
    ...
    def __call__(self, trial):
        ...
        if model_type == "RandomForest":
            ...
            model = lgb.LGBMClassifier(
                ...
                learning_rate=trial.suggest_float(
                    "lgb_learning_rate", 1e-3, 0.05, log=True
                ),
        ...
        for fold, (val_train_idx, val_test_idx) in enumerate(
            tscv.split(self.X_valid_full)
        ):
            X_train_fold = np.vstack(
                [self.X_train_full, self.X_valid_full[val_train_idx]]
            )
            y_train_fold = np.concatenate(
                [self.y_train_full, self.y_valid_full[val_train_idx]]
            )
            ...
            if sim_metrics["trades_count"] < 5:
                ...
                obj_score = sim_metrics["expectancy_per_trade"] - (
                    0.3 * sim_metrics["max_drawdown_pct"]
                )
...
def main():
    ...
    if "Datetime" in df_valid.columns:
        ...
        df_valid = df_valid[
            (df_valid["Datetime"].dt.year >= 2019)
            & (df_valid["Datetime"].dt.year <= 2021)
        ]
