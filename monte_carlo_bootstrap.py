...
def mc_bootstrap(trades_df, n_runs=10000, seed=42):
    ...
    for _ in range(n_runs):
        sample = rng.choice(trades_df['pnl_usd'].values, size=len(trades_df), replace=True)
    ...
    out = {
        ...
        'prob_ruina_20pct': (res_df['min_equity'] < (initial_balance * 0.8)).mean() * 100,
        'prob_ruina_50pct': (res_df['min_equity'] < (initial_balance * 0.5)).mean() * 100
    }
    return out
...
if __name__ == "__main__":
    try:
        ...
        print("❌ Archivo oos_trades.csv no encontrado. Ejecuta true_oos_test.py primero.")
