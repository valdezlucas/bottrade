...
def balance_dataset(df):
    ...
    print(
        f"    📏 Clases originales -> HOLD: {len(hold_df)}, BUY: {len(buy_df)}, SELL: {len(sell_df)}"
    )
    ...
    print(
        f"    ⚖️ Clases balanceadas -> HOLD: {len(hold_df)}, BUY: {len(buy_df)}, SELL: {len(sell_df)}\n"
    )
    return balanced_df
...
def main():
    ...
    parser.add_argument(
        "--tf", type=str, choices=["1d", "4h", "1h", "ALL"], default="1d"
    )
