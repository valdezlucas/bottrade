...
def run_blind_test(split_year=2024):
    ...
    for pair, config in PAIRS.items():
        if config.get("skip_backtest") and pair != "XAUUSD": continue
        ...
        df_oos = df_raw[df_raw["_date"] >= pd.Timestamp(f"{split_year}-01-01")].copy().reset_index(drop=True)
        ...
        for i in range(len(df) - 20):
            ...
            if pred == 0 or prob < threshold: continue
            
            signal = "BUY" if pred == 1 else "SELL"
            if signal == "SELL" and y_sell_proba[i] < 0.5: continue
            ...
            for _, f_row in future.iterrows():
                if signal == "BUY":
                    if f_row["Low"] <= sl: result = "SL"; exit_price = sl; break
                    if f_row["High"] >= tp: result = "TP"; exit_price = tp; break
                else:
        ...
        if pair_trades:
            ...
            print(f"  · {pair:6}: {stats['total_trades']:3} trades | WR: {stats['win_rate']:4.1f}% | PF: {stats['profit_factor']:.2f} | Exp: {stats['expectancy']:.1f} pips")
    ...
    print(f"  Esperanza Matemática: {global_stats['mathematical_expectancy']:.2f} pips/trade")
