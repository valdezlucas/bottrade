...
def load_and_prepare_data():
    try:
        ...
        df = pd.read_csv("df_1d_joined.csv")  # Usamos el dataset masivo que ya tenemos pre-descargado
    ...
    for i in range(n - lookahead):
        ...
        for j in range(i + 1, i + 1 + lookahead):
            ...
            if not hit_buy_tp and not hit_buy_sl:
                if l <= entry - sl_dist: hit_buy_sl = True
                elif h >= entry + tp_dist: hit_buy_tp = True
            ...
            if not hit_sell_tp and not hit_sell_sl:
                if h >= entry + sl_dist: hit_sell_sl = True
                elif l <= entry - tp_dist: hit_sell_tp = True
                    
            if not hit_sell_sl:
                if l < min_low: min_low = l

        if hit_buy_tp and not hit_buy_sl: targets[i] = 1
        elif hit_sell_tp and not hit_sell_sl: targets[i] = 2
...
def main():
    ...
    EXCLUDE_COLS = ["Datetime", "Date", "Open", "High", "Low", "Close", "Volume", "target", "potential_win", "potential_loss", "Asset", "mfe_buy", "mfe_sell", "fractal_high", "fractal_low"]
    ...
    clf = LGBMClassifier(n_estimators=300, learning_rate=0.05, max_depth=7, class_weight='balanced', random_state=42, device='gpu')
    ...
    reg_buy = LGBMRegressor(n_estimators=200, learning_rate=0.05, max_depth=6, random_state=42, device='gpu', objective='regression_l1')
    ...
    reg_sell = LGBMRegressor(n_estimators=200, learning_rate=0.05, max_depth=6, random_state=42, device='gpu', objective='regression_l1')
    ...
    print(f"      Predicciones de Extensión BUY:  {[f'{x:.2f} ATR' for x in pred_tps_buy]}")
    print(f"      Predicciones de Extensión SELL: {[f'{x:.2f} ATR' for x in pred_tps_sell]}")
